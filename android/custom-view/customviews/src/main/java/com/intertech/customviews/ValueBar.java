...
public class ValueBar extends View {
    ...
    private boolean circleTouched;
    private OnValueBarMoved valueBarMovedListener;
    ...
    private void drawBar(Canvas canvas) {
        ...
        float fillPosition = getFillPosition();
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()){
            case MotionEvent.ACTION_DOWN:
                if(isValueCircleTouched(event)){
                    circleTouched = true;
                }else if(isValueBarTouched(event)){
                   moveTo(event);
                }else{
                    circleTouched = false;
                }
    ...
    private int moveTo(MotionEvent event) {
        float left = getPaddingLeft();
        float right = left + getBarLength();

        float touchPoint = event.getX() - left;
        float totalBaxLengthInX = right - left;
        int value  = (int) ((touchPoint * maxValue)/totalBaxLengthInX);
        setValue(value);
        valueBarMovedListener.movedTo(value);
        invalidate();
        return value;
    }

    private boolean isValueCircleTouched(MotionEvent event) {
        float left = getFillPosition() - circleRadius;
        float right = getFillPosition() + circleRadius;
        float top = getBarCenter() - circleRadius;
        float bottom = getBarCenter() + circleRadius;

        float inpX = event.getX();
        float inpY = event.getY();

        if(inpX >= left && inpX <= right && inpY <= bottom && inpY >= top ){
            return true;
        }
    ...
    private boolean isValueBarTouched(MotionEvent event){
        float left = getPaddingLeft();
        float top = getBarCenter() - (barHeight / 2);
        float bottom = getBarCenter() + (barHeight / 2);
        float right = left + getBarLength();
    ...
    private float getBarLength(){
        ...
        return getWidth() - getPaddingRight() - getPaddingLeft() - circleRadius - maxValueRect.width() - spaceAfterBar;
    }
    ...
    public void setOnValueBarMoved(OnValueBarMoved obj){
        valueBarMovedListener = obj;
    }

    public interface OnValueBarMoved {
        void movedTo(int newPosition);
    }
}
